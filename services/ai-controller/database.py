...
SEED_PIPELINES = [
    {
        ...
        "docker_image": "pacs-demo/poc-xray:latest",
        "version": "1.0.0",
    ...
    },
    {
        ...
        "docker_image": "pacs-demo/poc-ct:latest",
        "version": "1.0.0",
    ...
    with get_db() as db:
        ...
        for p in SEED_PIPELINES:
            db.execute(
                ...
                " (id,name,description,docker_image,version,output_types,config_json)"
                " VALUES (:id,:name,:description,:docker_image,:version,:output_types,:config_json)",
                p,
            )
        for r in SEED_RULES:
            exists = db.execute(
            ...
            if not exists:
                db.execute(
                    "INSERT INTO rules"
                    " (pipeline_id,name,modality,body_part,auto_trigger,auto_saveback,saveback_types,priority,enabled)"
                    " VALUES (:pipeline_id,:name,:modality,:body_part,:auto_trigger,:auto_saveback,:saveback_types,:priority,:enabled)",
                    r,
                )
