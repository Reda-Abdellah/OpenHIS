...
def create_announcement(body: dict, claims: dict = Depends(require_token)):
...
def update_announcement(aid: int, body: dict,
    ...
    audit(username, action, target=str(aid))
    return row


@router.delete("/{aid}", status_code=204)
def delete_announcement(aid: int, claims: dict = Depends(require_token)):
    username = claims.get("preferred_username", "unknown")
    with get_db() as db:
        if not db.execute(
            "SELECT 1 FROM announcements WHERE id=?", (aid,)
    ...
    audit(username, "announcement-deleted", target=str(aid))
