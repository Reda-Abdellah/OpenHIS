...
- DEF-011: machine-token access to OpenMRS FHIR restored — the oauth2login module's `OAuth2ServiceAccountFilter` validates `Authorization: Bearer` JWTs (JWKS from `oauth2.properties` `keysUrl`) but requires a pre-existing OpenMRS user matching the token's `preferred_username`; a new `openmrs-init` one-shot container (emr profile) idempotently provisions `service-account-integration-hub-sa` (`infra/openmrs/init/10-service-accounts.sql`, password-less like the built-in `daemon` user). e2e S2.4/S2.6 now exercise the full report chain live (store → hub poll → OpenMRS push → audit)
- Lab-result reads target OpenELIS's backing FHIR store (`OPENELIS_FHIR_STORE_URL`, default `http://oe-fhir-store:8080/fhir`): OE's own FHIR servlet serves only Patient/Observation/Organization/Practitioner — DiagnosticReports surface exclusively on the store, where OE writes them
